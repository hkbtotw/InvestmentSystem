...
class Scraping_Price(object):
    ...
    def ScrapingOperation(self):
        ...
        for m in range(0,10):
            print(' Drive attemp :',m) 
            try:
                #driver=webdriver.Chrome('/usr/local/bin/chromedriver',options=option)
                driver = webdriver.Chrome(chrome_options=option)
                driver.implicitly_wait(30)
                print(' Drive successfully established ')
                break
            except:
                print(' Repeat acquiring drive ')
                time.sleep(10)
            ...
            for m in range(0,10):
                try:
                    ...
                    try:
                        ...
                        print("Loading  timed out. Killing Driver.")
                        print(te)
                        driver.close()  
        ...
        for n in range(0,10):
            ...
            try:
                ...
                try:
                    ...
                    print("Loading MSCIW timed out. Killing Driver.")
                    print(te)
                    driver.close() 
        ...
        msciwDf=msciwDf.append({'Name':'MSCIW','Price':float(msciwPrice), 'Update':msciwUpdatedate}, ignore_index=True)
        msciwDf=msciwDf.set_index(['Name'])

        #driver.quit()
        #display.stop()
        del driver, df
        return  dfOut, msciwDf
